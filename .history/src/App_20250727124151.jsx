...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const checkTelegramEnvironment = () => {
      // Strict Telegram environment check - require actual WebApp initialization
      const hasTelegramWebApp = window.Telegram?.WebApp
      const hasInitData = window.Telegram?.WebApp?.initData && window.Telegram.WebApp.initData.length > 0
      const isExpanded = window.Telegram?.WebApp?.isExpanded !== undefined
      
      // Check for telegram-specific user agent as secondary indicator
      const userAgent = navigator.userAgent.toLowerCase()
      const isTelegramUA = userAgent.includes('telegram')
      
      // Require BOTH WebApp object AND (initData OR proven Telegram UA)
      // This ensures we're actually running inside Telegram, not just a browser with Telegram script loaded
      const isTelegram = hasTelegramWebApp && (hasInitData || (isTelegramUA && isExpanded))
